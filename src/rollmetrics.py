...
from kinematic_solver_roll import out_dict_FL, out_dict_FR,out_dict_RL,out_dict_RR,arr1,arr2,res
...
rc_Z_pseudo=np.zeros(13)
rc_Y_pseudo=np.zeros(13)
rc_arr=np.zeros(13)
rc_arr[6]=res[1]  ##rc_static_height
for i in range(0,6,1):
    alpha=arr2[5-i]  #alpha val
    ...
    rc_Z_pseudo[i]=line_intersection((out_dict_FL['CP'][i,1],out_dict_FL['CP'][i,2]),(IC_1[0],IC_1[1]),(out_dict_FR['CP'][i,1],out_dict_FR['CP'][i,2]),(IC_2[0],IC_2[1]))[1]
    rc_Y_pseudo[i]=line_intersection((out_dict_FL['CP'][i,1],out_dict_FL['CP'][i,2]),(IC_1[0],IC_1[1]),(out_dict_FR['CP'][i,1],out_dict_FR['CP'][i,2]),(IC_2[0],IC_2[1]))[0]
    rc_arr[i]=((rc_Z_pseudo[i]-rc_Y_pseudo[i]*np.tan(np.radians(alpha))-rc_arr[6]*(1-(1/np.cos(np.radians(alpha))))))

for i in range(7,13,1):
    alpha=arr1[i-7]  #alpha val
    L_upp_pointer=inter_pt(out_dict_FL,i,'UBJ','UF','UA')
    L_low_pointer=inter_pt(out_dict_FL,i,'LBJ','LF','LA')
    IC_1=line_intersection((out_dict_FL['UBJ'][i,1],out_dict_FL['UBJ'][i,2]),(L_upp_pointer[0],L_upp_pointer[1]),(out_dict_FL['LBJ'][i,1],out_dict_FL['LBJ'][i,2]),(L_low_pointer[0],L_low_pointer[1]))
